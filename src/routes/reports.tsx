...
function ReportsPage() {
  ...
  const { data: tests } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("lab_tests")
        ...
        .gte("test_date", startDate).lte("test_date", endDate);
  ...
  const { data: funds } = useQuery({
    ...
    enabled: canFinance,
    queryFn: async () => {
      const { data, error } = await supabase.from("fund_utilizations")
        .select("*").gte("util_date", startDate).lte("util_date", endDate)
        .order("util_date");
      if (error) throw error;
      return data ?? [];
    },
  ...
  const { data: registrations } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("patient_registrations")
  ...
  const addFund = useMutation({
    mutationFn: async (f: { util_date: FormDataEntryValue | null; category: FormDataEntryValue | null; amount: number; notes: FormDataEntryValue | null }) => {
      const { error } = await supabase.from("fund_utilizations").insert(f as never);
      if (error) throw error;
    },
    onSuccess: () => { toast.success("Saved"); qc.invalidateQueries({ queryKey: ["report_funds"] }); setOpenFund(false); },
    onError: (e: Error) => toast.error(e.message),
  });

  // Aggregate per test type per month
  const summary = useMemo(() => {
    if (!tests) return { rows: [] as any[], totals: { total: 0, positive: 0, camp: 0, monthly: [0, 0, 0] } };
    const byTest: Record<string, { total: number; positive: number; camp: number; monthly: [number, number, number] }> = {};
    const totals = { total: 0, positive: 0, camp: 0, monthly: [0, 0, 0] as [number, number, number] };
    for (const t of tests) {
      const m = new Date(t.test_date).getMonth();
      const idx = q.months.indexOf(m);
      if (idx < 0) continue;
      if (!byTest[t.test_name]) byTest[t.test_name] = { total: 0, positive: 0, camp: 0, monthly: [0, 0, 0] };
      ...
      totals.total += 1; totals.monthly[idx] += 1;
      if (t.is_positive) totals.positive += 1;
      if (t.is_medical_camp) totals.camp += 1;
    }
    const rows = Object.entries(byTest).map(([name, v]) => ({ name, ...v })).sort((a, b) => b.total - a.total);
    return { rows, totals };
  }, [tests, q]);
  ...
  const exportExcel = () => {
    ...
    const header = ["Test", MONTH_NAMES[q.months[0]], MONTH_NAMES[q.months[1]], MONTH_NAMES[q.months[2]], "Total", "Positive", "Medical camp"];
    const sheetData = [
      [`LabTrack Quarterly Report — ${q.label} ${year}`],
      [],
      header,
      ...summary.rows.map((r) => [r.name, r.monthly[0], r.monthly[1], r.monthly[2], r.total, r.positive, r.camp]),
      ["TOTAL", summary.totals.monthly[0], summary.totals.monthly[1], summary.totals.monthly[2], summary.totals.total, summary.totals.positive, summary.totals.camp],
    ];
    const ws = XLSX.utils.aoa_to_sheet(sheetData);
    ws["!cols"] = [{ wch: 36 }, { wch: 10 }, { wch: 10 }, { wch: 10 }, { wch: 10 }, { wch: 10 }, { wch: 14 }];
  ...
  if (!canAny) {
    return <AccessDenied message="You don't have any reports assigned to your department. Ask an administrator to grant you the report permissions you need." />;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between gap-4 flex-wrap no-print">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-2"><BarChart3 className="h-7 w-7 text-primary" />Quarterly Reports</h1>
          <p className="mt-1 text-sm text-muted-foreground">Reports limited to your department. Print or export.</p>
        </div>
        <div className="flex flex-wrap gap-2 items-end">
          ...
          <div className="w-44"><Label className="text-xs">Quarter</Label>
            <Select value={String(quarter)} onValueChange={(v) => setQuarter(Number(v))}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>{QUARTERS.map((qq) => <SelectItem key={qq.v} value={String(qq.v)}>{qq.label}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <Button variant="outline" onClick={() => window.print()}><Printer className="mr-2 h-4 w-4" />Print</Button>
          {(canTests || canFinance) && <Button onClick={exportExcel}><FileDown className="mr-2 h-4 w-4" />Export Excel</Button>}
        </div>
      </div>

      <div className="hidden print:block text-center mb-6">
        <h1 className="text-2xl font-bold">LabTrack Quarterly Report</h1>
        <p className="text-sm">{q.label} — {year}</p>
      ...
      {canRegistrations && (
        <div className="rounded-xl border bg-card overflow-hidden">
          <div className="px-4 py-3 border-b font-semibold">Patient registrations — {q.label} {year}</div>
          ...
          <div className="grid gap-4 p-4 pt-0 md:grid-cols-2">
            <MiniTable title="By room / department" rows={Object.entries(regAgg.byRoom).sort((a, b) => b[1] - a[1])} />
      ...
      {canTests && (
        <div className="rounded-xl border bg-card overflow-hidden">
          <div className="px-4 py-3 border-b font-semibold">Tests summary — {q.label} {year}</div>
          <table className="w-full text-sm">
            <thead className="bg-muted/50 text-xs uppercase tracking-wide text-muted-foreground">
              <tr>
                <th className="px-4 py-2 text-left">Test</th>
                {q.months.map((m) => <th key={m} className="px-4 py-2 text-right">{MONTH_NAMES[m]}</th>)}
            ...
            <tbody className="divide-y">
              ...
              {summary.rows.length === 0 && <tr><td colSpan={7} className="p-6 text-center text-muted-foreground">No tests in this quarter.</td></tr>}
            </tbody>
        ...
        <div className="rounded-xl border bg-card overflow-hidden">
          <div className="px-4 py-3 border-b flex items-center justify-between">
            ...
            <Dialog open={openFund} onOpenChange={setOpenFund}>
              <DialogTrigger asChild><Button size="sm" className="no-print"><Plus className="mr-1 h-3 w-3" />Add entry</Button></DialogTrigger>
              <DialogContent>
                <DialogHeader><DialogTitle>Fund utilization</DialogTitle></DialogHeader>
                <form onSubmit={(e) => {
                  e.preventDefault();
                  ...
                  <div><Label>Date *</Label><Input type="date" name="util_date" required defaultValue={new Date().toISOString().slice(0, 10)} /></div>
                  <div><Label>Category *</Label><Input name="category" required placeholder="Reagents, Equipment, Salaries…" /></div>
                  <div><Label>Amount *</Label><Input type="number" step="0.01" name="amount" required /></div>
                  <div><Label>Notes</Label><Textarea name="notes" /></div>
                  <DialogFooter><Button type="submit" disabled={addFund.isPending}>Save</Button></DialogFooter>
                </form>
              </DialogContent>
            </Dialog>
          </div>
          <table className="w-full text-sm">
            <thead className="bg-muted/50 text-xs uppercase tracking-wide text-muted-foreground">
              <tr><th className="px-4 py-2 text-left">Date</th><th className="px-4 py-2 text-left">Category</th><th className="px-4 py-2 text-right">Amount</th><th className="px-4 py-2 text-left">Notes</th></tr>
            </thead>
            <tbody className="divide-y">
              {funds?.map((f) => (
                <tr key={f.id}>
              ...
              {funds?.length === 0 && <tr><td colSpan={4} className="p-6 text-center text-muted-foreground">No entries.</td></tr>}
            </tbody>
            {(funds?.length ?? 0) > 0 && (
              <tfoot className="bg-muted/30 font-semibold">
                <tr><td className="px-4 py-2" colSpan={2}>TOTAL</td><td className="px-4 py-2 text-right font-mono">{totalFunds.toFixed(2)}</td><td /></tr>
              </tfoot>
            )}
          </table>
        </div>
      ...
      {canStock && (
        <div className="rounded-xl border bg-card overflow-hidden">
          <div className="px-4 py-3 border-b font-semibold">Deliveries — {q.label} {year}</div>
          <table className="w-full text-sm">
            <thead className="bg-muted/50 text-xs uppercase tracking-wide text-muted-foreground">
              <tr>
            ...
            <tbody className="divide-y">
              {(stockMoves ?? []).map((d, i) => (
                <tr key={i}>
                  ...
                  <td className="px-4 py-2 text-right">{Number(d.quantity)} {d.unit ?? ""}</td>
                  <td className="px-4 py-2 text-muted-foreground">{d.supplier}</td>
                </tr>
              ))}
              {(stockMoves?.length ?? 0) === 0 && <tr><td colSpan={4} className="p-6 text-center text-muted-foreground">No deliveries in this quarter.</td></tr>}
            </tbody>
          </table>
        </div>
      )}
      ...
      <div className="rounded-md border border-dashed bg-muted/20 p-3 text-xs text-muted-foreground flex items-center gap-2 no-print">
        ...
        Only the reports your department has access to are shown. Admins can change this in Permissions.
      </div>
    </div>
  );
}
...
function Stat({ label, value, tone }: { label: string; value: string; tone?: "emerald" | "rose" }) {
  const c = tone === "emerald" ? "text-emerald-600" : tone === "rose" ? "text-rose-600" : "text-foreground";
  return (
...
function MiniTable({ title, rows }: { title: string; rows: [string, number][] }) {
  return (
    <div className="rounded-lg border">
      <div className="border-b bg-muted/40 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">{title}</div>
      <table className="w-full text-sm">
        <tbody className="divide-y">
          {rows.length === 0 && <tr><td className="px-3 py-2 text-muted-foreground">No data.</td></tr>}
          {rows.map(([k, v]) => (
            <tr key={k}><td className="px-3 py-2">{k}</td><td className="px-3 py-2 text-right font-semibold tabular-nums">{v}</td></tr>
          ))}
        </tbody>
      </table>
    </div>
