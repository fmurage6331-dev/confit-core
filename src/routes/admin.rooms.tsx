      ...
      <div className="mb-6 flex items-end justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Manage rooms and which users can access each one.
          </p>
        </div>
            ...
            {rows.map((r) => (
              <tr key={r.id} className="border-t">
                ...
                <td className="px-4 py-3 text-right">
                  <Button variant="ghost" size="sm" onClick={() => setAccessRoom(r)}>
          ...
          {editing && (
            <form onSubmit={onSubmit} className="space-y-4">
              ...
              <div>
                ...
                <Select
                  value={editing.kind}
                  onValueChange={(v) => setEditing({ ...editing, kind: v as RoomKind })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="flex items-center justify-between rounded-lg border p-3">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    Only active rooms appear in registration.
                  </div>
                </div>
                <Switch
    ...
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-md">
        ...
        <p className="text-xs text-muted-foreground">
          Admins always have access. Tick users who may enter this room.
        </p>
        <div className="max-h-96 overflow-y-auto rounded-md border">
          {loading && <div className="p-4 text-sm text-muted-foreground">Loading…</div>}
          {!loading && list.length === 0 && (
            <div className="p-4 text-sm text-muted-foreground">No non-admin users.</div>
          )}
          {!loading &&
            list.map((u) => (
              <label
                key={u.id}
                className="flex cursor-pointer items-center justify-between gap-3 border-b px-3 py-2 last:border-b-0 hover:bg-accent"
              >
