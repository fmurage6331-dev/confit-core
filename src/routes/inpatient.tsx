...
function InpatientGate() {
  ...
  const canView = hasPerm("admissions_view") || hasPerm("admit_patient") || hasPerm("bed_management");
  if (!canView) return <AccessDenied />;
  return <Inpatient />;
}

type Ward = { id: string; name: string; ward_type: string | null; section: string | null; floor: string | null; daily_rate: number | null };
...
function Inpatient() {
  ...
  const canDischarge = hasPerm("discharge_patient") || hasPerm("bed_management") || hasPerm("admit_patient");
  ...
  const wardsQ = useQuery({
    queryKey: ["ipd-wards"],
    queryFn: async () => {
      const { data, error } = await supabase.from("wards").select("id,name,ward_type,section,floor,daily_rate").eq("is_active", true).order("name");
  ...
  const bedsQ = useQuery({
    queryKey: ["ipd-beds"],
    queryFn: async () => {
      const { data, error } = await supabase.from("beds").select("id,ward_id,bed_number,status").order("bed_number");
      if (error) throw error;
      return (data ?? []) as Bed[];
    },
  });
  ...
  const admissionsQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("admissions")
        .select("id,patient_id,bed_id,ward_id,encounter_id,admitted_at,expected_discharge_date,discharged_at,admitting_doctor,admission_reason,admission_type,status,patients(id,patient_name,file_number),wards(id,name),beds(id,bed_number)")
        .eq("status", "admitted")
  ...
  const admissionByBed = useMemo(() => {
    ...
    (admissionsQ.data ?? []).forEach((a) => { if (a.bed_id) m.set(a.bed_id, a); });
    return m;
  }, [admissionsQ.data]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-2xl font-semibold flex items-center gap-2"><BedDouble className="h-6 w-6 text-primary" /> Inpatient (IPD)</h1>
          <p className="text-sm text-muted-foreground">Manage ward beds, admissions and discharges.</p>
        </div>
      ...
      <Tabs defaultValue="beds">
            ...
            <div className="space-y-8">
              {groupBySection(wardsQ.data ?? []).map(([sectionKey, sectionWards]) => (
                <section key={sectionKey} className="space-y-3">
                  <h2 className="text-lg font-semibold border-b pb-1">{sectionLabel(sectionKey)}</h2>
                          ...
                          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                            <div>
                              ...
                              <div className="text-xs text-muted-foreground">
                                {w.floor && `Floor ${w.floor} · `}{occ}/{beds.length} occupied{w.daily_rate ? ` · KES ${w.daily_rate}/day` : ""}
                              </div>
                            </div>
                            {canManageBeds && (
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2">
                              {beds.map((b) => (
                                ...
                                >
                                  ...
                                  <div className="text-[10px] uppercase tracking-wide opacity-80">{b.status ?? "?"}</div>
                                </button>
                              ))}
                            </div>
                          )}
              ...
              {(wardsQ.data ?? []).length === 0 && <div className="text-sm text-muted-foreground">No wards configured.</div>}
              <Legend />
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="inpatients" className="mt-4">
          <CurrentInpatients rows={admissionsQ.data ?? []} loading={admissionsQ.isLoading}
            canDischarge={canDischarge} onDischarged={refreshAll} />
        </TabsContent>
      </Tabs>
          ...
          {selectedBed && (() => {
            ...
            return (
              <div className="space-y-3 text-sm">
                <div>Status: <Badge className={badgeClass(status)}>{status}</Badge></div>
                {admission && admission.patients ? (
                  <div className="rounded-md border p-3 space-y-1">
                    <div className="font-medium">{admission.patients.patient_name ?? "Patient"}</div>
                    {admission.patients.file_number && <div className="text-xs text-muted-foreground">File #{admission.patients.file_number}</div>}
                    <div className="text-xs">Admitted: {fmt(admission.admitted_at)}</div>
                    {admission.admitting_doctor && <div className="text-xs">Doctor: {admission.admitting_doctor}</div>}
                  ...
                  <Button onClick={() => { setAdmitOpen(true); setSelectedBed(null); }}>Admit patient to this bed</Button>
                ) : (
...
function bedColor(status: string | null) {
  switch (status) {
    case "available": return "bg-emerald-500/10 border-emerald-500/40 text-emerald-700 hover:bg-emerald-500/20 dark:text-emerald-300";
    case "occupied": return "bg-red-500/10 border-red-500/40 text-red-700 hover:bg-red-500/20 dark:text-red-300";
    case "cleaning": return "bg-amber-500/10 border-amber-500/40 text-amber-700 hover:bg-amber-500/20 dark:text-amber-300";
    case "maintenance": return "bg-slate-500/10 border-slate-500/40 text-slate-700 hover:bg-slate-500/20 dark:text-slate-300";
    default: return "bg-muted border-border text-muted-foreground hover:bg-muted/70";
  }
}
function badgeClass(status: string) {
  switch (status) {
    case "available": return "bg-emerald-500/15 text-emerald-700 dark:text-emerald-300";
    case "occupied": return "bg-red-500/15 text-red-700 dark:text-red-300";
    case "cleaning": return "bg-amber-500/15 text-amber-700 dark:text-amber-300";
    case "maintenance": return "bg-slate-500/15 text-slate-700 dark:text-slate-300";
...
export function DischargeButton({ admissionId, encounterId, onDone }: { admissionId: string; encounterId: string | null; onDone: () => void }) {
  const { user } = useAuth();
  ...
  async function submit() {
    ...
    if (!body) { toast.error("Discharge summary is required."); return; }
    setBusy(true);
    ...
    if (noteErr) { setBusy(false); toast.error(noteErr.message); return; }
    // 2) Then discharge
    ...
    setBusy(false);
    if (error) {
      toast.error(/discharge summary/i.test(error.message) ? error.message : `Failed to discharge: ${error.message}`);
      return;
    }
  ...
  return (
    <>
      <Button size="sm" variant="destructive" onClick={() => setOpen(true)}>Discharge</Button>
      <Dialog open={open} onOpenChange={(o) => { if (!busy) setOpen(o); }}>
        <DialogContent>
          <DialogHeader><DialogTitle>Discharge patient</DialogTitle></DialogHeader>
          <div className="space-y-2">
            <Label>Discharge summary <span className="text-rose-600">*</span></Label>
            ...
            <p className="text-xs text-muted-foreground">A discharge summary is required. The bed will be freed automatically.</p>
          </div>
          <DialogFooter>
...
function CurrentInpatients({ rows, loading, canDischarge, onDischarged }: {
  ...
  if (rows.length === 0) return <div className="text-sm text-muted-foreground">No current inpatients.</div>;
  return (
          ...
          {rows.map((a) => (
            <tr key={a.id} className="border-t">
              <td className="p-3">
                ...
                {a.patients?.file_number && <div className="text-xs text-muted-foreground">#{a.patients.file_number}</div>}
              </td>
...
type PatientOpt = { id: string; patient_name: string | null; file_number: string | null; phone: string | null };

function AdmitDialog({ open, onOpenChange, wards, beds, onAdmitted }: {
  open: boolean; onOpenChange: (o: boolean) => void; wards: Ward[]; beds: Bed[]; onAdmitted: () => void;
}) {
  const qc = useQueryClient();
  ...
  const patientsQ = useQuery({
    ...
    queryFn: async () => {
      ...
      let q = supabase.from("patients").select("id,patient_name,file_number,phone").order("created_at", { ascending: false }).limit(20);
      if (term) {
        const like = `%${term}%`;
        q = q.or(`file_number.ilike.${like},patient_name.ilike.${like},first_name.ilike.${like},family_name.ilike.${like},phone.ilike.${like}`);
      }
  ...
  function reset() {
    setWardId(""); setBedId(""); setPatientId(""); setPatientSearch("");
    setDoctor(""); setReason(""); setType("elective"); setExpected("");
  }

  async function submit() {
    if (!patientId) { toast.error("Select a patient"); return; }
    if (!wardId || !bedId) { toast.error("Select a ward and bed"); return; }
    setBusy(true);
        ...
        <div className="space-y-3">
          <div>
            ...
            <div className="relative">
              ...
              <Input value={patientSearch} onChange={(e) => { setPatientSearch(e.target.value); setPatientId(""); }} placeholder="Search by name, file # or phone" className="pl-9" />
            </div>
            <div className="mt-2 max-h-40 overflow-y-auto rounded-md border">
              {(patientsQ.data ?? []).map((p) => (
                <button
                  key={p.id}
                  type="button"
                  onClick={() => { setPatientId(p.id); setPatientSearch(p.patient_name ?? ""); }}
                ...
                >
                  ...
                  <span className="text-xs text-muted-foreground">{p.file_number ? `#${p.file_number}` : p.phone ?? ""}</span>
                </button>
              ))}
              {(patientsQ.data ?? []).length === 0 && <div className="px-3 py-2 text-xs text-muted-foreground">No matches.</div>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Select value={bedId} onValueChange={setBedId} disabled={!wardId}>
                ...
                <SelectContent>
                  {availableBeds.map((b) => <SelectItem key={b.id} value={b.id}>{b.bed_number}</SelectItem>)}
                  {wardId && availableBeds.length === 0 && <div className="px-3 py-2 text-xs text-muted-foreground">No available beds.</div>}
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input value={doctor} onChange={(e) => setDoctor(e.target.value)} placeholder="Dr. ..." />
            </div>
            <div>
...
function groupBySection(wards: Ward[]): Array<[string, Ward[]]> {
  ...
  return SECTION_ORDER
    ...
    .concat(
      Array.from(map.entries()).filter(([k]) => !SECTION_ORDER.includes(k as typeof SECTION_ORDER[number])) as Array<[string, Ward[]]>
    );
}
function sectionLabel(key: string) {
  switch (key) {
    case "female": return "Female Wards";
    case "male": return "Male Wards";
    case "paediatrics": return "Paediatrics (under 13)";
    default: return "Other Wards";
  }
}

function BedManageControls({ ward, beds, onChanged }: { ward: Ward; beds: Bed[]; onChanged: () => void }) {
  const [busy, setBusy] = useState(false);
  ...
  async function removeLastFreeBed() {
    ...
    if (!removable) { toast.error("No free bed to remove (occupied beds can't be removed)."); return; }
  ...
  return (
    <div className="flex items-center gap-2">
      <Button size="sm" variant="outline" disabled={busy} onClick={removeLastFreeBed}>− Remove bed</Button>
      <Button size="sm" disabled={busy} onClick={addBed}>+ Add bed</Button>
    </div>
  );
}
