...
export const Route = createFileRoute("/records/new")({
  validateSearch: (s: Record<string, unknown>) =>
    ({ reg: typeof s.reg === "string" ? s.reg : undefined }) as { reg?: string },
  component: () => (
    <AppShell>
      <PermGuard perm="records_create">
        <NewRecord />
      </PermGuard>
    </AppShell>
  ),
...
function NewRecord() {
  ...
  const [form, setForm] = useState({
    patient_name: "",
    age: "",
    registration_number: search.reg ?? "",
    lab_number: "",
    ...
    (async () => {
      ...
      if (!row) {
        setClearance({
          state: "unknown",
          message:
            "No registration found for this number. The lab can only run tests requested by reception.",
        });
        return;
      }
      const requested =
      ...
      const base = {
        ...
        fromRoom: row.from_room ?? undefined,
        requestedTests: requested,
      };
      if (settled)
        setClearance({
          state: "cleared",
          message: `${row.patient_name} cleared by accounting${row.from_room ? ` · from ${row.from_room}` : ""}.`,
          ...base,
        });
      else
        setClearance({
          state: "blocked",
          message: `${row.patient_name} has not been cleared (${row.payment_status}). Send to accounting before running tests.`,
          ...base,
        });
    })();
    return () => {
  ...
  useEffect(() => {
    ...
    fetchTemplateFor(effectiveTestName)
      .then((tpl) => {
        if (cancelled) return;
        setTemplate(tpl);
        setStructured(
  ...
  async function onSubmit(e: FormEvent) {
    ...
    if (clearance.state === "blocked") {
      toast.error(
        "Patient not cleared by accounting. Tests cannot be run until payment is settled.",
      );
      return;
    }
    if (clearance.state === "unknown" && !isMedicalCamp) {
      toast.error(
        "This test was not requested by reception. The lab can only run tests requested for a registered patient (or tick 'Medical camp').",
      );
      return;
    }
    ...
    if (
      ...
      clearance.requestedTests &&
      clearance.requestedTests.length > 0
    ) {
      const ok = clearance.requestedTests.some((t) => t.name === test_name);
      if (!ok) {
        toast.error(
          `This test was not requested at reception. Allowed: ${clearance.requestedTests.map((t) => t.name).join(", ")}`,
        );
        return;
      }
    }
    const resultText = template && structured ? JSON.stringify(structured) : form.result;
    ...
    if (
      ...
      if (doneError) {
        ...
        if (stillPending.length > 0) {
          toast.success(
            `Record saved — ${stillPending.length} more requested test(s) still pending before the patient is routed back.`,
          );
        } else {
          ...
          if (routeError) {
            ...
            toast.success(
              "Record saved — all requested tests complete, patient routed back to requesting room",
            );
          }
        }
      }
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <form
        onSubmit={onSubmit}
        className="space-y-6 rounded-xl border bg-card p-6 shadow-[var(--shadow-card)]"
      >
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            ...
            {clearance.state !== "idle" && (
              ...
              >
                {clearance.state === "checking"
                  ? "Checking accounting clearance…"
                  : clearance.message}
              </div>
            )}
          </div>
            ...
            {clearance.requestedTests && clearance.requestedTests.length > 0 ? (
              <>
                <Select value={testChoice} onValueChange={handleTestChange}>
                  <SelectTrigger>
                    <SelectValue placeholder="Select a requested test" />
                  </SelectTrigger>
                  <SelectContent>
                    {clearance.requestedTests.map((t) => (
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Limited to tests reception requested for this patient
                  {clearance.fromRoom ? ` from ${clearance.fromRoom}` : ""}.
                </p>
              </>
            ) : (
              <>
        ...
        <div className="flex justify-end gap-2">
          <Button type="button" variant="ghost" onClick={() => navigate({ to: "/records" })}>
            Cancel
          </Button>
          <Button type="submit" disabled={submitting || clearance.state === "blocked"}>
