...
type Patient = {
  id: string; file_number: string | null; patient_name: string | null;
  first_name: string | null; middle_name: string | null; family_name: string | null;
  sex: string | null; date_of_birth: string | null; estimated_age: number | null;
  phone: string | null; email: string | null;
  address_line1: string | null; city: string | null; county: string | null; country: string | null;
  occupation: string | null; marital_status: string | null; nationality: string | null;
  next_of_kin: unknown;
  is_deceased: boolean | null; date_of_death: string | null;
  created_at: string;
};

type Encounter = {
  id: string; created_at: string; status: string | null;
  payment_mode: string | null; payment_status: string | null;
  subtotal: number | null; patient_due: number | null; amount_paid: number | null;
  from_room: string | null; current_room_id: string | null;
  tests: unknown;
...
function PatientProfile() {
  ...
  const { data: encounters, isLoading: eLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("encounters")
        .select("id,created_at,status,payment_mode,payment_status,subtotal,patient_due,amount_paid,from_room,current_room_id,tests,encounter_type")
        .eq("patient_id", id)
  ...
  const name = patient.patient_name || [patient.first_name, patient.middle_name, patient.family_name].filter(Boolean).join(" ") || "Unnamed";
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <div className="rounded-xl border bg-card p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
              {patient.file_number && <span className="font-mono">File #{patient.file_number}</span>}
              {patient.sex && <Badge variant="outline" className="capitalize">{patient.sex}</Badge>}
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 text-sm">
          ...
          <Info label="Address" value={[patient.address_line1, patient.city, patient.county, patient.country].filter(Boolean).join(", ") || null} />
          <Info label="Next of kin" value={kok?.name ? `${kok.name}${kok.relation ? ` (${kok.relation})` : ""}${kok.phone ? ` · ${kok.phone}` : ""}` : null} />
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              {eLoading && <tr><td colSpan={7} className="p-6 text-center text-muted-foreground">Loading…</td></tr>}
              {!eLoading && encounters?.length === 0 && (
                <tr><td colSpan={7} className="p-6 text-center text-muted-foreground">No encounters yet.</td></tr>
              )}
              {encounters?.map((e) => {
                ...
                return (
                  <tr key={e.id} className="hover:bg-accent/40">
                    ...
                    <td className="px-4 py-3">
                      ...
                      {e.payment_status && <span className="ml-1 text-xs text-muted-foreground">· {e.payment_status}</span>}
                    </td>
                    <td className="px-4 py-3 text-right tabular-nums">KSh {Number(e.subtotal ?? 0).toFixed(2)}</td>
                    <td className={`px-4 py-3 text-right tabular-nums ${bal > 0 ? "text-destructive" : ""}`}>KSh {bal.toFixed(2)}</td>
                    <td className="px-4 py-3 text-right">
                      <Button asChild size="sm" variant="ghost">
                        <Link to="/records/$id" params={{ id: e.id }}>Open</Link>
                      </Button>
                    </td>
                  </tr>
                );
...
function NewEncounterDialog({ patientId, patientName }: { patientId: string; patientName: string }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (!open) return;
    supabase.from("insurance_providers").select("id,name,code,coverage_percentage").eq("is_active", true).order("name")
      .then(({ data }) => setInsurers((data ?? []) as Insurer[]));
    supabase.from("lab_test_catalog").select("id,name,price,cash_price,insurance_price,kind,category").eq("is_active", true).order("name")
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      const { error, data } = await supabase.from("encounters").insert({
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button><Plus className="mr-2 h-4 w-4" /> New encounter</Button>
      </DialogTrigger>
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              <ModeBtn label="Cash" icon={Banknote} active={mode === "cash"} on={() => setMode("cash")} cls="bg-emerald-600" />
              <ModeBtn label="Insurance" icon={Shield} active={mode === "insurance"} on={() => setMode("insurance")} cls="bg-blue-600" />
              <ModeBtn label="Free / Waived" icon={HeartHandshake} active={mode === "free"} on={() => setMode("free")} cls="bg-amber-500" />
            </div>
            {mode === "insurance" && (
          ...
          <div>
            ...
            <div className="max-h-60 overflow-y-auto rounded-md border">
              {tests.length === 0 && <p className="p-3 text-sm text-muted-foreground">No services configured.</p>}
                  ...
                  return (
                    ...
                    >
                      <span>
                        ...
                        <span className="ml-1 text-[10px] uppercase text-muted-foreground">{t.category || t.kind}</span>
                      </span>
                      <span className="tabular-nums text-xs text-muted-foreground">KSh {priceFor(t).toFixed(2)}</span>
                    </button>
                  );
                })}
              </div>
...
function ModeBtn({ label, icon: Icon, active, on, cls }: {
  label: string; icon: typeof Banknote; active: boolean; on: () => void; cls: string;
}) {
  return (
    <button
      type="button"
