...
export const Route = createFileRoute("/admin/services")({
  component: () => <AppShell><Guard allow={["admin"]}><ServicesAdmin /></Guard></AppShell>,
});
...
function ServicesAdmin() {
  ...
  const save = useMutation({
    mutationFn: async (v: Partial<Service>) => {
      ...
      if (editing) {
        const { error } = await supabase.from("lab_test_catalog").update(payload).eq("id", editing.id);
        if (error) throw error;
      } else {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="flex items-end justify-between">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-2"><Stethoscope className="h-7 w-7 text-primary" />Services & charges</h1>
          <p className="mt-1 text-sm text-muted-foreground">Manage services, lab tests, procedures and their charges per payment mode.</p>
        </div>
        <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) setEditing(null); }}>
          <DialogTrigger asChild>
            <Button onClick={() => setEditing(null)}><Plus className="mr-2 h-4 w-4" />Add service</Button>
          </DialogTrigger>
          <ServiceDialog
            initial={editing}
      ...
      <div className="overflow-hidden rounded-xl border bg-card">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y">
            ...
            {rows?.length === 0 && <tr><td colSpan={7} className="p-6 text-center text-muted-foreground">No services yet.</td></tr>}
            {rows?.map((r) => (
              <tr key={r.id}>
                <td className="px-4 py-3 font-medium">{r.name}</td>
                <td className="px-4 py-3"><Badge variant="outline" className="capitalize">{r.kind}</Badge></td>
                <td className="px-4 py-3 text-muted-foreground">{r.category || "—"}</td>
                <td className="px-4 py-3 text-right tabular-nums">KSh {Number(r.cash_price ?? r.price ?? 0).toFixed(2)}</td>
                <td className="px-4 py-3 text-right tabular-nums text-muted-foreground">
                  {r.insurance_price != null ? `KSh ${Number(r.insurance_price).toFixed(2)}` : "—"}
                </td>
                <td className="px-4 py-3">{r.is_active ? <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100">Active</Badge> : <Badge variant="secondary">Off</Badge>}</td>
...
function ServiceDialog({ initial, onSubmit, pending }: {
  ...
  return (
    <DialogContent>
      <DialogHeader><DialogTitle>{initial ? "Edit service" : "Add service"}</DialogTitle></DialogHeader>
      <form onSubmit={(e) => {
        e.preventDefault();
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div><Label>Category</Label><Input name="category" defaultValue={initial?.category ?? ""} placeholder="e.g. Chemistry" /></div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label>Cash price (KSh) *</Label>
            <Input type="number" step="0.01" name="cash_price" required defaultValue={initial?.cash_price ?? initial?.price ?? 0} />
          </div>
          <div>
            <Label>Insurance price (KSh)</Label>
            <Input type="number" step="0.01" name="insurance_price" defaultValue={initial?.insurance_price ?? ""} placeholder="Optional" />
          </div>
        </div>
