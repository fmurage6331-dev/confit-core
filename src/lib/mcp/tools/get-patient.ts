...
export default defineTool({
  ...
  title: "Get patient",
  description:
    "Fetch a patient registration by file number, including requested services, associated lab tests, and prescriptions.",
  inputSchema: {
  ...
  handler: async ({ file_number }, ctx) => {
    ...
    if (!reg)
      return {
        content: [{ type: "text", text: `No patient found with file ${file_number}` }],
        isError: true,
      };
    const [{ data: tests }, { data: rx }] = await Promise.all([
      sb
        .from("lab_tests")
        .select("id, test_name, test_date, result, is_positive")
