...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (user && (user.user_metadata as { must_change_password?: boolean } | undefined)?.must_change_password
    ...
    (async () => {
      if (isAdmin) {
        ...
        const { data } = await supabase
          ...
          .eq("user_id", user.id);
        const rows = ((data ?? []) as unknown as { rooms: { id: string; name: string; is_active: boolean } | null }[])
          .map((r) => r.rooms).filter((r): r is { id: string; name: string; is_active: boolean } => !!r && r.is_active);
  ...
  const canAnyReport =
    hasPerm("reports.registrations") || hasPerm("reports.tests")
    || hasPerm("reports.finance") || hasPerm("reports.stock");


  // Primary sidebar: day-to-day workflow only.
  const primaryNav = [
    { to: "/dashboard", label: "Dashboard", icon: LayoutDashboard, show: true },
    { to: "/patients", label: "Patients", icon: Contact, show: hasPerm("register_patient") || hasPerm("records_view") },
    { to: "/register-patient", label: "Registration", icon: UserPlus, show: hasPerm("register_patient") },
  ...
  const roomNav = accessibleRooms.map((r) => ({
    to: `/rooms/${r.id}`, label: r.name, icon: DoorOpen,
  }));

  const nav = [...primaryNav, ...roomNav] as ReadonlyArray<{ to: string; label: string; icon: typeof LayoutDashboard }>;
      ...
      <aside className="fixed inset-y-0 left-0 hidden w-60 border-r bg-sidebar md:flex md:flex-col no-print">
        <div className="flex items-center gap-2 px-4 py-5">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-primary text-primary-foreground overflow-hidden">
            {logoUrl ? <img src={logoUrl} alt="" className="h-full w-full object-contain" /> : <FlaskConical className="h-5 w-5" />}
          </div>
          <span className="min-w-0 flex-1 truncate font-semibold">{appName}</span>
          {moduleItems.length > 0 && (
            <ModulesLauncher items={moduleItems} />
          )}
        </div>
        <nav className="flex-1 space-y-1 px-3 overflow-y-auto">
          {nav.map(({ to, label, icon: Icon }) => {
            ...
            return (
              <Link key={to} to={to as "/dashboard"}
        ...
        <div className="border-t p-3">
          <div className="px-3 pb-2 text-xs text-muted-foreground truncate">
            {user.email}{isAdmin && <span className="ml-1 rounded bg-primary/10 px-1.5 py-0.5 text-[10px] text-primary">admin</span>}
          </div>
          <Link to="/account" className="flex items-center gap-2 rounded-md px-3 py-2 text-sm hover:bg-sidebar-accent">
      ...
      <div className="md:pl-60">
        <header className="flex items-center justify-between border-b px-6 py-3 md:hidden no-print">
          <div className="flex items-center gap-2 min-w-0">
            {logoUrl ? <img src={logoUrl} alt="" className="h-5 w-5 object-contain" /> : <FlaskConical className="h-5 w-5 text-primary" />}
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0" title="Modules" aria-label="Open modules">
      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="relative">
          ...
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search for a module" className="pl-9" />
        </div>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 max-h-[60vh] overflow-y-auto pr-1">
          ...
          {filtered.length === 0 && (
            <div className="col-span-full py-10 text-center text-sm text-muted-foreground">No modules match.</div>
          )}
        </div>
      </DialogContent>
    </Dialog>
